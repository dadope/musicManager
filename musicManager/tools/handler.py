...
class mediaHandler:
    def __init__(self, songs:list, playlist:dict, orderToPlay:str=None, artist:str=None, cli:bool=True, infinite_loop=False):
        """
        ...
        if self.artist:
            ...
            if len(self.songs) == 0:
                ...
                collector.log_and_exit(f"Could not find artist {self.artist}", 1)
        ...
        if len(songs) == 0:
            ...
            collector.log_and_exit("no songs to be played", 1)
        ...
        # lets the player loop infinitely
        if infinite_loop:
        ...
        def listPlayerCallback(event):
            if self.curr_song_percentage != 0:
                collector.log_play_percentage(self.currFile, self.curr_song_percentage)
    ...
    # converts file uri (returned by vlc) to a parsable filename
